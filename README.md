<div align="center">

<img src="https://raw.githubusercontent.com/Jai-saraswat/Jai-saraswat/main/assets/neon-pulse-banner.svg"
alt="Jai Saraswat"
width="100%">

<br>

<img src="https://raw.githubusercontent.com/Jai-saraswat/Jai-saraswat/main/assets/typewriter-intro.svg"
alt="Jai Saraswat"
width="100%">

<br>

<a href="https://github.com/Jai-saraswat">
<img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=FFFFFF" alt="GitHub">
</a>
<a href="https://www.linkedin.com/in/jai-saraswat-877809284/">
<img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=0A66C2" alt="LinkedIn">
</a>
<a href="https://leetcode.com/u/codeZENZZ/">
<img src="https://img.shields.io/badge/LeetCode-0D1117?style=for-the-badge&logo=leetcode&logoColor=FFA116" alt="LeetCode">
</a>
<a href="https://www.instagram.com/jai_saraswatt/">
<img src="https://img.shields.io/badge/Instagram-0D1117?style=for-the-badge&logo=instagram&logoColor=E4405F" alt="Instagram">
</a>
<a href="mailto:saraswatjai1409@gmail.com">
<img src="https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=EA4335" alt="Email">
</a>

</div>

---

# Profile

**Computer Science & Engineering (Data Science)** student focused on **Artificial Intelligence, Machine Learning, Computer Vision, Data Science, and Software Development**.

I build practical applications, experiment with intelligent systems, and strengthen my problem-solving skills through **DSA, machine learning, and hands-on software development**.

---

# Tech Stack

### Languages

<div align="left">

<img src="https://skillicons.dev/icons?i=python,cpp,js,html,css&perline=8&theme=dark"
alt="Languages">

</div>

### Frameworks & Libraries

<div align="left">

<img src="https://skillicons.dev/icons?i=react,fastapi,pytorch,tensorflow,opencv,numpy,pandas&perline=8&theme=dark"
alt="Frameworks and libraries">

</div>

### Tools & Development

<div align="left">

<img src="https://skillicons.dev/icons?i=git,github,vscode,pycharm,linux&perline=8&theme=dark"
alt="Development tools">

</div>

# Technologies & Frameworks

<div align="left">

| **Languages** | **Frameworks & Libraries** | **AI / Data** | **Tools & Platforms** |
|:---:|:---:|:---:|:---:|
| Python | FastAPI | PyTorch | Git |
| C++ | Flask | TensorFlow | GitHub |
| Java | Streamlit | OpenCV | PyCharm |
| JavaScript | Gradio | NumPy | VS Code |
| HTML | LangChain | Pandas | Linux |
| CSS | LangGraph | Scikit-learn | PostgreSQL |
| SQL | LangSmith | Matplotlib | Neo4j |

</div>

# Contributions

<div align="center">

<picture>
  <source
    media="(prefers-color-scheme: dark)"
    srcset="https://raw.githubusercontent.com/Jai-saraswat/Jai-saraswat/output/snake-dark.svg">
  <source
    media="(prefers-color-scheme: light)"
    srcset="https://raw.githubusercontent.com/Jai-saraswat/Jai-saraswat/output/snake.svg">
  <img
    src="https://raw.githubusercontent.com/Jai-saraswat/Jai-saraswat/output/snake.svg"
    alt="GitHub contribution snake"
    width="100%">
</picture>

<br>

### `learn()` → `build()` → `debug()` → `improve()`

</div>
